...
import { Grade } from "../../Pages/SignUp/SignUp";
...
const Profile = () => {
	...
	const fetchGrades = async () => {
		const response = await axios.get("/account/grades/");
		...
		<Card className="w-auto drop-shadow-2xl">
			...
			<CardBody className="grid grid-cols-2 gap-10 p-10 pt-5 ">
				...
				>
					...
					/>
					<select
						className="w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:border-2 focus:border-blue-500 text-base font-medium text-gray-700"
						name="grade"
						value={userForm.grade}
						onChange={handleChange}
					>
						{grades.map((grade) => (
							<option value={grade.id.toString()}>{grade.name}</option>
						))}
					</select>
